...
function OrderPreparingGrocery(props) {
	...
	const [options, setOptions] = React.useState([
		Constants.ORDER_STATUS_STR_SELECT,
		Constants.ORDER_STATUS_STR_PREPARED,
	]);
	const [value, setValue] = React.useState(options[1]);

	function handleChangeValue(e, index, item) {
		let STATUS = Utility.getStatusKeyGrocery(e);
		if (STATUS == Constants.ORDER_STATUS_STR_SELECT) return;
		let req = {
			[Constants.KEY_UNDERSCORE_ID]: item[Constants.KEY_UNDERSCORE_ID],
	...
	function navigateOrderDetails(item) {
		...
		context.props.history.push({
			pathname: Utility.isAdmin()
				? Constants.SCREEN_ORDER_DETAILS_ADMIN_GROCERY
...
OrderPreparingGrocery.propTypes = {
	classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(OrderPreparingGrocery);
