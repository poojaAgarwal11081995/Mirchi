...
import {
	reqChangeStatusDriverRequestedGrocery,
	reqListRequestedGrocery,
	reqChangeStatusGroceryRequested,
	reqGetGroceryList,
	reqDeleteRestro,
	reqListReqDriversGrocery,
	reqDeleteDriverGrocery,
} from "../../actions";
...
import DashboardGroceryItem from "../../tables/DashboardGroceryItem";
import RequestedDriverTableGrocery from "../../tables/RequestedDriverTableGrocery";

let selectedTab = 0;
let userData = undefined;
class DashboardGrocery extends BaseComponent {
	constructor(props) {
		super(props);
		...
		this.getGroceryList();
		this.getDriverList();
		console.log("user_data rahul", this.props);
	}
	...
	updateStatus = (request) => {
		...
		this.props.reqChangeStatusGroceryRequested(request, this);
	};

	updateDriverStatus = (request) => {
		request[Constants.KEY_CREATED_BY] = userData[Constants.KEY_USERID];
		this.props.reqChangeStatusDriverRequestedGrocery(request, this);
	};
				...
				<Fragment>
					...
					<Tabs
						...
						<Tab
							style={{
								backgroundColor:
									selectedTab == 0 ? Colors.colorBalckTrans : Colors.white,
							}}
							label={this.strings(StringKeys.Requested_Grocery)}
							icon={<PhoneCallbackIcon />}
						/>
						<Tab
							style={{
								backgroundColor:
									selectedTab == 1 ? Colors.colorBalckTrans : Colors.white,
							}}
							label={this.strings(StringKeys.Requested_Driver)}
							icon={<PhoneCallbackIcon />}
						/>
					</Tabs>
					...
					this.state.headerData.length > 0 ? (
						<div className={classes.tableContainer}>
							{selectedTab == 0 ? (
								...
								<RequestedDriverTableGrocery
									data={this.state[Constants.KEY_DRIVER_LIST]}
									context={this}
								/>
							)}
						</div>
	...
	getGroceryList = () => {
		...
		this.props.reqListRequestedGrocery(data, this);
	};
	...
	getDriverList = () => {
		...
		this.props.reqListReqDriversGrocery(data, this);
	};
	...
	navigateDriverDetails(id) {
		this.props.history.push({
			pathname: Constants.SCREEN_REQUESTED_DRIVER_DETAILS_GROCERY,
			[Constants.KEY_DRIVER_ID]: id,
		});
	}

	navigateGroceryDetails(id) {
		this.props.history.push({
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_LIST_REQUESTED_GROCERY) {
				respObj = {
				...
				nextProps[Constants.KEY_TYPE] === types.API_LIST_REQ_DRIVER_GROCERY
			) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
					[Constants.KEY_DRIVER_LIST]:
...
export default connect(mapStateToProps, {
	...
	reqListReqDriversGrocery,
	reqDeleteDriverGrocery,
})(withStyles(styles)(DashboardGrocery));
