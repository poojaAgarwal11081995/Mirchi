...
import {
	reqEditDriverGrocery,
	reqDriverDetailsGrocery,
	reqRegionList,
...
class DriverDetailsGrocery extends BaseComponent {
	constructor(props) {
		super(props);
		this.state = {
		...
		if (this.state[Constants.KEY_SHOW_PROGRESS] == true) {
			...
			return (
				<Fragment>
					<div
						style={{
							padding: DRIVER_ID != undefined && DRIVER_ID != null ? 0 : 20,
						}}>
						...
						<div class="justify-content-start" style={{ marginTop: 20 }}>
							<div class="row">
								<div class="col-sm ">
									<CommonGridTextField
										required
		...
		var data = {
			...
			[Constants.KEY_REGION_ID]:
				this.state.selectedRegion != undefined
					...
					: undefined,
			[Constants.KEY_OFFICE_ID]:
				this.state.selectedOffice != undefined
					? this.state.selectedOffice[Constants.KEY_UNDERSCORE_ID]
					: undefined,
	...
	editDrriverReq = (data) => {
		...
		this.props.reqEditDriverGrocery(data, this);
	};
	...
	getDriverDetailsReq = () => {
		...
		this.props.reqDriverDetailsGrocery(data, this);
	};
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_DRIVER_DETAILS_GROCERY) {
				...
				for (const key in officeList) {
					if (officeList.hasOwnProperty(key)) {
						...
						if (
							objDriver[Constants.KEY_OFFICE_DETAIL] != undefined &&
							element[Constants.KEY_UNDERSCORE_ID] ==
								objDriver[Constants.KEY_OFFICE_DETAIL][
									Constants.KEY_UNDERSCORE_ID
								]
						) {
				...
				respObj = {
					...
					phone: objDriver.phone,
					city: objDriver.city,
					zip: objDriver.zip,
					state: objDriver.state,
					address: objDriver.address,
					country: objDriver.country,
					image: objDriver.image,
					...
					[Constants.KEY_LICENSE_NUMBER]:
						objDriver[Constants.KEY_LICENSE_NUMBER],
					[Constants.KEY_LICENSE_EXPIRY_DATE]:
						objDriver[Constants.KEY_LICENSE_EXPIRY_DATE],
					...
					[Constants.KEY_AADHAR_IMAGE_NAME]:
						objDriver[Constants.KEY_AADHAR_IMAGE],
					[Constants.KEY_PAN_IMAGE_NAME]: objDriver[Constants.KEY_PAN_IMAGE],
					[Constants.KEY_LICENSE_IMAGE_NAME]:
						objDriver[Constants.KEY_LICENSE_IMAGE],
					...
					[Constants.ACCOUNT_HOLDER_NAME]:
						objDriver[Constants.ACCOUNT_HOLDER_NAME],
				...
				nextProps[Constants.KEY_TYPE] === types.API_EDIT_DRIVER_GROCERY
			) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
					disabledClickedBtn: false,
...
DriverDetailsGrocery.propTypes = {
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(DriverDetailsGrocery));
