...
import { reqListGroceryEarning } from '../../actions';
...
let objGrocery = undefined;
let thisWeekData = undefined;

class GroceryEarning extends BaseComponent {
    constructor(props) {
        super(props)
        userData = CustomStorage.getSessionDataAsObject(Constants.KEY_USER_DATA);
        objGrocery = CustomStorage.getSessionDataAsObject(Constants.KEY_GROCERY_DETAILS);
        this.state = {
    ...
    listDriverEarning = () => {
        ...
        this.props.reqListGroceryEarning({
            [Constants.KEY_GROCERY_ID]: !Utility.isAdmin() ? userData[Constants.KEY_GROCERY_DETAILS][Constants.KEY_UNDERSCORE_ID] :
                objGrocery[Constants.KEY_UNDERSCORE_ID]
        }, this)
    }
    render() {
                                ...
                                return (
                                    <Card key={index} xs={4} style={{ backgroundColor: Colors.white, marginRight: 5 }}>
                                        <ListItem onClick={() => {
                                            ...
                                            this.navigateGroceryEarning(item)
                                        }
                                        }>
                                            <Grid container
                                                direction="row"
    ...
    handleResponse = (nextProps) => {
        ...
        if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
            ...
            if (nextProps[Constants.KEY_TYPE] === types.API_LIST_GROCERY_EARNING) {
                respObj = {
                    [Constants.KEY_SHOW_PROGRESS]: false,

                };

                respObj.dataLength = nextProps[Constants.KEY_RESPONSE][Constants.KEY_DATA][Constants.KEY_GROCERY_EARNING_LIST].length;
                respObj.earningDriverList = this.setEarningListData(
                    nextProps[Constants.KEY_RESPONSE][Constants.KEY_DATA]
                    [Constants.KEY_GROCERY_EARNING_LIST],
                    nextProps[Constants.KEY_RESPONSE][Constants.KEY_DATA]
    ...
    navigateGroceryEarning(item) {
        this.props.history.push({
            pathname: Utility.isAdmin() ? Constants.SCREEN_GROCERY_EARNING_TIMEWISE_IN_ADMIN : Constants.SCREEN_GROCERY_EARNING_TIMEWISE,
        });
    }
...
export default connect(mapStateToProps, { reqListGroceryEarning })(withStyles(styles)(GroceryEarning));
