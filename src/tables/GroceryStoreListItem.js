...
function GroceryStoreListItem(props) {
	...
	function handleClose(option, pos) {
		...
		if (option != undefined) {
			...
			//     CustomStorage.setSessionDataAsObject(
			//         Constants.KEY_GROCERY_ID, n[Constants.KEY_UNDERSCORE_ID]);
			//         context.props.history.push({
			//             pathname: Utility.isAdmin ? Constants.SCREEN_RATING_AND_REVIEW_ADMIN_GROCERY :
			//                 Constants.SCREEN_RATING_AND_REVIEW_GROCERY,
			//             [Constants.KEY_GROCERY_DETAILS]: n
			//         });
			// }
			// rating & review option end //
			else if (option == Constants.ORDER_HISTORY) {
				CustomStorage.setSessionDataAsObject(Constants.KEY_GROCERY_DETAILS, n);
				CustomStorage.setSessionDataAsObject(
				...
				context.props.history.push({
					pathname: Utility.isAdmin
						? Constants.SCREEN_ORDER_LIST_GROCERY
						: Constants.SCREEN_ORDER_DETAILS_GROCERY,
					[Constants.KEY_GROCERY_DETAILS]: n,
				});
			} else if (option == Constants.PRODUCT_CATE_TYPE) {
				CustomStorage.setSessionDataAsObject(
				...
				context.props.history.push({
					pathname: Utility.isAdmin
						? Constants.SCREEN_PRODUCT_LIST_GROCERY_ADMIN
						: Constants.SCREEN_PRODUCT_LIST_GROCERY,
					[Constants.KEY_GROCERY_DETAILS]: n,
				});
			} else if (option == Constants.EARNING_GROCERY) {
				CustomStorage.setSessionDataAsObject(
					Constants.KEY_GROCERY_DETAILS,
					undefined,
				);
				...
				<TableHead>
					<TableRow>
						<TableCell align="justify" style={CommonStyle.tableRowHeader}>
							{context.strings(StringKeys.Store_Name)}
						</TableCell>
								...
								.map((n, index) => (
									<TableRow key={n.id}>
											...
											<TableCell align="center">
												{
													<IconButton onClick={() => {}} aria-label="Check">
														{n[Constants.KEY_SHOP_LICENCE_STATUS] ==
															Constants.DRIVER_STATUS_APPROVED_DOC &&
														n[Constants.KEY_GSTN_OR_PAN_STATUS] ==
															Constants.DRIVER_STATUS_APPROVED_DOC ? (
															<ActiveIcon />
														) : (
															<InActiveIcon />
										...
										<TableCell align="center">
											{[
												,
												// <Tooltip title={context.strings("Delete")}>
												// 	<IconButton
												// 		aria-label="Delete"
												// 		onClick={() => {
												// 			context.deleteGrocery(n);
												// 		}}>
												// 		<DeleteIcon />
												// 	</IconButton>
												// </Tooltip>,
												<Tooltip
													title={context.strings(StringKeys.ViewAndEdit)}>
													<IconButton
														aria-label="Detail"
														onClick={() => {
															CustomStorage.setSessionDataAsObject(
	...
	function handleResponse(nextProps) {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			// alert(nextProps[Constants.KEY_SHOW_PROGRESS]);
			respObj = {
...
GroceryStoreListItem.propTypes = {
	classes: PropTypes.object.isRequired,
};
function mapStateToProps({ response }) {
	return response;
}
export default withStyles(styles)(GroceryStoreListItem);
