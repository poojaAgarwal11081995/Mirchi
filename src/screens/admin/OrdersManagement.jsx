...
import { reqOrderListForAdmin } from "../../actions";
...
import AdminAllOrdersItem from "../admin/orderreuest/AdminAllOrdersItem";
...
class OrdersManagement extends BaseComponent {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			[Constants.KEY_SEARCH_DRIVER_NAME]: undefined,

			[Constants.KEY_SEARCH_]: undefined,
			selectedOption: undefined,
			status_array: [
				{
					[Constants.KEY_LABEL]: Constants.ORDER_STATUS_STR_PENDING,
					[Constants.KEY_VALUE]: Constants.ORDER_STATUS_PENDING,
				},
				{
				...
				{
					[Constants.KEY_LABEL]: Constants.ORDER_STATUS_STR_CANCEL_BY_CUSTOMER,
					[Constants.KEY_VALUE]: Constants.ORDER_STATUS_STR_CANCEL_BY_CUSTOMER,
				},
			],
		};
		this.getOrdersList();
	...
	handleResetData = () => {
		this.setState({
			...
			[Constants.KEY_SEARCH_DRIVER_NAME]: "",
							...
							<div class="col-sm ">
								<Grid style={{ marginTop: 20 }}>
									<CommonGridTextField
										xs={TEXTFIELD_XS}
										style={{ marginTop: 20 }}
										id="searchByDrivername"
										label={"Delivery Man"}
										fullWidth
										className={classes.textField}
										value={this.state[Constants.KEY_SEARCH_DRIVER_NAME]}
										onChange={this.handleSearchChange(
											Constants.KEY_SEARCH_DRIVER_NAME,
										)}
										autoComplete="resname"
							...
							<div class="col-sm ">
								<Grid style={{ marginTop: 20 }}>
									<CommoanAutocomplete
										...
										options={this.state.status_array}
										components={animatedComponents}
										onChange={this.handleChangeStatus}
										placeholder={this.strings(StringKeys.Select_Status)}
									/>
								</Grid>
							</div>
						</div>
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_ORDER_LIST_FOR_ADMIN) {
				respObj = {
	...
	getOrdersList = () => {
		...
		if (
			...
			data[Constants.KEY_SEARCH_ORDER] = Utility.removeWhiteSpace(
				this.state[Constants.KEY_SEARCH_ORDER],
			);
		}
		if (
			this.state[Constants.KEY_SEARCH_DRIVER_NAME] != undefined &&
			this.state[Constants.KEY_SEARCH_DRIVER_NAME] != null &&
			this.state[Constants.KEY_SEARCH_DRIVER_NAME] != ""
		) {
			data[Constants.KEY_SEARCH_DRIVER_NAME] = Utility.removeWhiteSpace(
				this.state[Constants.KEY_SEARCH_DRIVER_NAME],
			);
		}

		console.log("reset Data", data, this);
		this.props.reqOrderListForAdmin(data, this);
	};
...
export default connect(mapStateToProps, { reqOrderListForAdmin })(
	withStyles(styles)(OrdersManagement),
);
