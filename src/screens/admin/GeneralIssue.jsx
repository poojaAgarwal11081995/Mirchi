...
import {
	...
	reqStateList,
	reqCityList,
	reqRestroDocStatusChange,
	reqChangeStatusRestro,
	reqGetRestoList,
	reqDeleteRestro,
} from "../../actions";
import GeneralIssueList from "../../tables/GeneralIssueList";
...
const issueList = [
	{ q: "I have a query related to placing an order" },
	{ q: "I am unable to log in on Swiggy" },
	{ q: "I have a payment or refund related query" },
	{ q: "I have a coupon related query" },
	{ q: "I want to unsubscribe from Swiggy communications" },
];

class GeneralIssue extends BaseComponent {
	constructor(props) {
		super(props);
		userData = CustomStorage.getSessionDataAsObject(Constants.KEY_USER_DATA);
		this.state = {
			[Constants.KEY_SHOW_PROGRESS]: true,
			[Constants.KEY_DATA]: issueList,
			headerData: [
		...
		if (this.state[Constants.KEY_SHOW_PROGRESS] == true) {
			...
			return (
				<Fragment>
					...
					<Grid item xs={TEXTFIELD_XS} style={{ marginTop: 0 }}>
						<CommonButton
							...
							onClick={() => {
								...
								this.setState({ addNewIssue: !this.state.addNewIssue });
							}}
					...
					/>
					<AlertDialogSlide
						open={this.state.addNewIssue}
						toggle={(bool) => {
							this.setState({ addNewIssue: bool });
						}}
					/>
				</Fragment>
			);
	...
	viewImage = (image, index, isThis) => {
		...
		let data = [
			...
			{
				[Constants.KEY_UNDERSCORE_ID]: image[Constants.KEY_UNDERSCORE_ID],
				[Constants.KEY_NAME]: this.strings(StringKeys.FSSAI_Licence_Image),
				[Constants.KEY_STATUS]: image[Constants.KEY_FSSAI_STATUS],
				[Constants.KEY_IMAGE]: image[Constants.KEY_FSSAI_LICENCE_IMG],
				[Constants.KEY_KEY]: Constants.KEY_FSSAI_STATUS,
			},
			{
				[Constants.KEY_UNDERSCORE_ID]: image[Constants.KEY_UNDERSCORE_ID],
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_RESTRO_LIST) {
				...
				nextProps[Constants.KEY_TYPE] === types.API_CHANGE_STATUS_RESTRO
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				this.getRestoList();
			} else if (
				nextProps[Constants.KEY_TYPE] === types.API_RESTRO_DOC_STATUS_CHANGE
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				this.getRestoList();
...
export default connect(mapStateToProps, {
	...
	reqGetRestoList,
	reqDeleteRestro,
})(withStyles(styles)(GeneralIssue));
