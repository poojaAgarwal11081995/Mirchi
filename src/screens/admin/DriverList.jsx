...
class DriverList extends BaseComponent {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			deleted_msg: this.strings(StringKeys.Delete_Driver_Msg),
			...
			return (
				<Fragment>
					...
					<CommonGridTextField
						...
						style={{ marginTop: 20 }}
						id="restaurantName"
						label={this.strings(StringKeys.Search_by_driver_name)}
						fullWidth
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
			if (nextProps[Constants.KEY_TYPE] === types.API_DRIVER_LIST) {
				respObj = {
				...
				this.setState(respObj, () => {
					if (changeAbleDriverId >= 0) {
						console.log("dvncnvdjskxcnvkjd ", changeAbleDriverId + "");
	...
	getDriverList = () => {
		console.log("ghghghgh", this.state[Constants.KEY_SEARCH]);
		let data = {
			offset: 0,
		};
	...
	navigateEarringPage() {
		// alert('esds')
		this.props.history.push({
			pathname: Constants.SCREEN_EARRING_PAGE,
		});
	}

	updateStatus = (request) => {
		let data = {
			[Constants.KEY_UNDERSCORE_ID]: request[Constants.KEY_UNDERSCORE_ID],
			[Constants.KEY_STATUS]: request[Constants.KEY_STATUS],
		};
