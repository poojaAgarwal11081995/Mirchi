...
class AddDiscount extends BaseComponent {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			selectedProductType: "",
			[Constants.KEY_VALID_FROM]: null, //Utility.convertDate(new Date()),
			[Constants.KEY_VALID_TO]: null, //Utility.convertDate(new Date()),
		};
	}
	componentDidMount() {
		if (
	...
	checkVaidation = (event) => {
		...
		if (Utility.checkVaidationAddDiscountRes(data, this)) {
	...
	addDiscountReq = (data) => {
		...
		if (
			...
			this.props.reqEditDiscount(data, this);
		} else {
			this.props.reqAddDiscount(data, this);
		}
	};
	...
	getDiscountDetails = (data) => {
		...
		this.props.reqDiscountDetails(data, this);
	};

	handleResponse = (nextProps) => {
		var mes = nextProps["response"];
		console.log("hjgdfhjghjdgshdshds", mes);
		var respObj = null;
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (
				nextProps[Constants.KEY_TYPE] === types.API_ADD_DISCOUNT ||
				nextProps[Constants.KEY_TYPE] === types.API_EDIT_DISCOUNT
			) {
				if (nextProps[Constants.KEY_TYPE] === types.API_ADD_DISCOUNT) {
					respObj = {
						[Constants.KEY_SHOW_PROGRESS]: false,
						disabledClickedBtn: false,
					};
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_DISCOUNT_DETAILS) {
				if (
