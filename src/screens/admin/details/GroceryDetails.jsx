...
class GroceryDetails extends BaseComponent {
	constructor(props) {
		super(props);
		this.state = {
		...
		try {
			GROCERY_ID = CustomStorage.getSessionDataAsObject(
	...
	addGroceryReq = (data) => {
		...
		if (Utility.checkGroceryDetails(data, this)) {
	...
	handleResponse = (nextProps) => {
		console.log("NEXTPROPS2222222", nextProps);
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (
				...
				let allStoreType = obj[Constants.KEY_STORE_TYPE_ID];

				// ? this.state.storeTypeList[0] : obj[Constants.KEY_STORE_TYPE] == Constants.KEY_STORE_TYPE_ID
				//     ? this.state.storeTypeList[1] : this.state.storeTypeList[2]

				console.warn(
					"KEY_STORE_TYPE_ID::::::::::::::ll::::::::::::",
					allStoreType,
					obj[Constants.KEY_STORE_TYPE_ID],
				);
				let list_store = this.state["storeTypeList"];
				if (this.state["storeTypeList"] != undefined && list_store.length > 0) {
					for (let element of list_store) {
						if (obj[Constants.KEY_STORE_TYPE_ID] != undefined) {
							console.log("asdsadaid", element[Constants.KEY_UNDERSCORE_ID]);
							if (
								obj[Constants.KEY_STORE_TYPE_ID] ===
								element[Constants.KEY_UNDERSCORE_ID]
							) {
								console.log("found", "sada");
								this.setState({
									selectedStoreType: element,
								});
							}
						}
					}
				...
				respObj = {
					...
					// [Constants.KEY_STORE_TYPE_ID]: obj[Constants.KEY_STORE_TYPE_ID],
					[Constants.KEY_STORE_TYPE_ID]: allStoreType,
				...
				console.warn(
					"KEY_STORE_TYPE_ID:22:::::::::::::::::::::::::",
					this.state[Constants.KEY_STORE_TYPE_ID],
				);
				for (const element in allStoreType) {
					if (this.setState[Constants.KEY_STORE_TYPE_ID] != undefined) {
						if (
							this.state[Constants.KEY_STORE_TYPE_ID] ==
							element[Constants.KEY_UNDERSCORE_ID]
						) {
				...
				toast("Grocery Vendor List Updated Successfully");
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(GroceryDetails));
