...
class RestauentDetails extends BaseComponent {
	constructor(props) {
		super(props);
		...
		try {
			RESTO_ID = CustomStorage.getSessionDataAsObject(
							...
							<div class="row">
								<div class="col-sm ">
									<CommonMultipleSelectorImg
										...
										value={
											this.state[Constants.KEY_KITCHEN_IMG_NAME] != undefined &&
											this.state[Constants.KEY_KITCHEN_IMG_NAME] != null &&
											this.state[Constants.KEY_KITCHEN_IMG_NAME] != "" &&
											this.state[Constants.KEY_KITCHEN_IMG_NAME].toString()
										}
										variant="outlined"
										onChange={this.kitchanImagePress}
										clearPress={() => {
											//alert(JSON.stringify(this.state[Constants.KEY_KITCHEN_IMG]))
							...
							<div class="row">
								...
								<div class="col-sm ">
									<CommonMultipleSelectorImg
										...
										value={this.state[Constants.KEY_DINING_PACKAGING_IMG_NAME]}
										variant="outlined"
	...
	checkVaidation = (event) => {
		...
		console.log("gagaggagagaggag", data, this);
		console.log("VALIDATION++++++++", Utility.checkVaidation(data, this));
		// if (Utility.checkVaidation(data, this)) {
		// 	alert("1");
	...
	addRestaurentReq = (data) => {
		// alert("addRequest");
		if (!this.isForAdd()) {
			data[Constants.KEY_UNDERSCORE_ID] = RESTO_ID;
		}
		...
		if (
			this.state[Constants.KEY_KITCHEN_IMG_UPLOAD] !== undefined &&
			this.state[Constants.KEY_KITCHEN_IMG_UPLOAD] !== null &&
			this.state[Constants.KEY_KITCHEN_IMG_UPLOAD] !== ""
		) {
		...
		if (
			...
			this.state[Constants.KEY_FSSAI_LICENCE_IMG_UPLOAD] !== ""
		) {
		...
		if (
			...
			this.state[Constants.KEY_DINING_PACKAGING_IMG_UPLOAD] !== ""
		) {
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (
				...
				// CustomStorage.setSessionDataAsObject("user_data", respObj);
				this.setState(respObj);
				this.getCountryList();
				this.getCatoryList();
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(RestauentDetails));
