...
let TEXTFIELD_XS = 12;
class OfficeList extends BaseComponent {
	constructor(props) {
		super(props);
	...
	navigateAddPage() {
		this.props.history.push({
			pathname: Constants.SCREEN_ADD_OFFICE,
		});
	}
	...
	onOfficeEditClick = (officeItem) => {
		...
		this.props.history.push({
			pathname: Constants.SCREEN_EDIT_OFFICE,
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_OFFICE_LIST) {
				...
				nextProps[Constants.KEY_TYPE] === types.API_OFFICE_STATUS_CHANGE
			) {
