...
import {
	...
	reqEditProductGrocery,
	reqGetCategoryListGrocery,
	reqProductDetailsGrocery,
} from "../../../actions";
...
let productDetailsGrocery = undefined;

class ProductDetailsGrocery extends BaseComponent {
	constructor(props) {
		super(props);
		this.state = {
			...
			// productTypeArray: [{ label: Constants.KEY_VEG, [Constants.KEY_VALUE]: Constants.RESTRO_TYPE_VEG },
			// { label: Constants.KEY_NON_VEG, [Constants.KEY_VALUE]: Constants.RESTRO_TYPE_NON_VEG }],
			selectedProductType: "",
			[Constants.KEY_WEIGHT]: "",
			[Constants.KEY_QUANTITY]: "",
		};
		productDetailsGrocery = CustomStorage.getSessionDataAsObject(
	...
	getProductDetailsReq = () => {
		var data = {
			[Constants.KEY_UNDERSCORE_ID]:
				productDetailsGrocery[Constants.KEY_UNDERSCORE_ID],
		};
		this.props.reqProductDetailsGrocery(data, this);
	};

	getProductCatoryList = () => {
		let data = {
			[Constants.KEY_USERID]: productDetailsGrocery[Constants.KEY_GROCERY_ID],
		};
	...
	checkVaidation = (event) => {
		...
		var data = {
			...
			[Constants.KEY_DISCOUNT_IN_PERCENT]:
				...
				[Constants.KEY_DISCOUNT_IN_PERCENT] != undefined &&
			...
			// [Constants.KEY_PRODUCT_TYPE]: this.state.selectedProductType != undefined &&
			//     this.state.selectedProductType != '' ? this.state.selectedProductType[Constants.KEY_VALUE] : '',
			[Constants.KEY_GROCERY_ID]:
				productDetailsGrocery[Constants.KEY_GROCERY_ID],
		...
		// if (Utility.checkVaidationAddProduct(data, this)) {
	...
	editProductReq = (data) => {
		...
		// if (data[Constants.KEY_PRODUCT_CATE_ID] != undefined && data[Constants.KEY_PRODUCT_CATE_ID] != '') {
		//     this.props.reqEditProductGrocery(data, this)
		// } else {
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			// 	toastStyle,
			// );
			if (nextProps[Constants.KEY_TYPE] === types.API_EDIT_PRODUCT_GROCERY) {
				...
				// if (
				// 	productDetailsGrocery != undefined &&
				// 	productDetailsGrocery != null
				// ) {
				// 	toast(
				// 		nextProps[Constants.KEY_RESPONSE][Constants.KEY_MESSAGE],
				// 		toastStyle,
				...
				nextProps[Constants.KEY_TYPE] === types.API_PRODUCT_DETAILS_GROCERY
			) {
				let updatedArray = "";
				let catArray = [];
				...
				respObj = {
					...
					[Constants.KEY_WEIGHT]: obj[Constants.KEY_WEIGHT],
					[Constants.KEY_QUANTITY]: obj[Constants.KEY_QUANTITY],
...
ProductDetailsGrocery.propTypes = {
...
export default connect(mapStateToProps, {
	...
	reqEditProductGrocery,
	reqProductDetailsGrocery,
})(withStyles(styles)(ProductDetailsGrocery));
