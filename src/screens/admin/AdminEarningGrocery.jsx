...
import { reqListAdminEarningGrocery } from '../../actions';
...
class AdminEarningGrocery extends BaseComponent {
    constructor(props) {
        super(props)
    ...
    listDriverEarning = () => {
        this.props.reqListAdminEarningGrocery({
        }, this)
    }
    render() {
        ...
        if (this.state[Constants.KEY_SHOW_PROGRESS] == true) {
            ...
            if ((this.state.adminEarningList == undefined || this.state.adminEarningList == '' || this.state.adminEarningList == null) || this.state.adminEarningList.length <= 0)
                return (
                    <Fragment>
    ...
    handleResponse = (nextProps) => {
        ...
        if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
            ...
            if (nextProps[Constants.KEY_TYPE] === types.API_LIST_ADMIN_EARNING_GROCERY) {
                respObj = {
                    [Constants.KEY_SHOW_PROGRESS]: false,

                };

                respObj.dataLength = nextProps[Constants.KEY_RESPONSE][Constants.KEY_DATA][Constants.KEY_ADMIN_EARNING_LIST].length;
    ...
    navigateRestaurentEarning(item) {
        this.props.history.push({
            pathname: Constants.SCREEN_ADMIN_EARNING_TIMEWISE_GROCERY,
        });
    }
...
export default connect(mapStateToProps, { reqListAdminEarningGrocery })(withStyles(styles)(AdminEarningGrocery));
