...
import { reqListRestroEarningTimeWise } from '../../actions';
...
class EarringTimeWiseRestro extends BaseComponent {
    constructor(props) {
        super(props)
    ...
    handleClick = index => {
        var list = this.state[Constants.KEY_RESTRO_EARNING_LIST];
        list[index].isOpen = list[index].isOpen != undefined ? !list[index].isOpen : true
        this.setState({
            [Constants.KEY_RESTRO_EARNING_LIST]: list
        })
                ...
                <Fragment>
                    <div className={classes.appBarSpacer} style={{ paddingTop: 50, }}>
                        ...
                        <List style={{ display: "flex" }}>
                            {this.state[Constants.KEY_RESTRO_EARNING_LIST].map((item, index) => {
    ...
    handleResponse = (nextProps) => {
        ...
        if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
            ...
            if (nextProps[Constants.KEY_TYPE] == types.API_RESTRO_EARNING_PER_WEEK) {
                ...
                if (nextProps[Constants.KEY_RESPONSE].hasOwnProperty(Constants.KEY_DATA)) {
                    ...
                    [Constants.KEY_DATA][Constants.KEY_RESTRO_EARNING_LIST];

                    respObj[Constants.KEY_RESTRO_EARNING_LIST] = nextProps[Constants.KEY_RESPONSE]
                    [Constants.KEY_DATA][Constants.KEY_RESTRO_EARNING_LIST];
    ...
    listDriverEarning = () => {
        this.props.reqListRestroEarningTimeWise({
            [Constants.KEY_RESTO_ID]: earningDetails[Constants.KEY_RESTO_ID],
...
EarringTimeWiseRestro.propTypes = {
...
export default connect(mapStateToProps, { reqListRestroEarningTimeWise })(withStyles(styles)(EarringTimeWiseRestro));
