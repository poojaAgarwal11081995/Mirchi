...
import {
	...
	reqPaidDriverPayment,
	reqRegionList,
	reqCountryList,
	reqStateList,
	reqCityList,
	reqNeedToPayForAllDrivers,
} from "../../actions";
...
class DriverPaymentHistory extends BaseComponent {
	constructor(props) {
		super(props);
	...
	firstBtnClick = () => {
		...
		this.props.reqPaidDriverPayment(
			{
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
			if (
				nextProps[Constants.KEY_TYPE] === types.API_NEED_TO_PAY_FOR_ALL_DRIVERS
			) {
				respObj = {
				...
				nextProps[Constants.KEY_TYPE] === types.API_PAID_DRIVER_PAYMENT
			) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
				};
	...
	getDriverList = () => {
		console.log(this.state[Constants.KEY_SEARCH]);
		let data = {
			offset: 0,
		};
		...
		this.props.reqNeedToPayForAllDrivers(data, this);
	};
...
DriverPaymentHistory.propTypes = {
...
export default connect(mapStateToProps, {
	...
	reqCityList,
	reqNeedToPayForAllDrivers,
})(withStyles(styles)(DriverPaymentHistory));
