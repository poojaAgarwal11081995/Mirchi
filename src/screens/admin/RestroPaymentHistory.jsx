...
import {
	...
	reqNeedToPayForAllRestro,
	reqPaidRestroPayment,
	reqRegionList,
	reqCountryList,
	reqStateList,
	reqCityList,
...
class RestroPaymentHistory extends BaseComponent {
	constructor(props) {
		super(props);
				...
				<Fragment>
					...
					{selctedFloatingCashItem !== undefined && (
						<YesNoDialog
							...
							title={[Constants.KEY_APP_NAME_STRING]}
							content={` Given  Amount to Restaurant ${
								selctedFloatingCashItem[Constants.KEY_RETSRO_DETAILS][
									Constants.KEY_NAME
								]
							}`}
	...
	handleResponse = (nextProps) => {
		console.log("nextProps5888888", nextProps);
		var respObj = null;
		...
		if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
			if (
				nextProps[Constants.KEY_TYPE] === types.API_NEED_TO_PAY_FOR_ALL_RESTRO
			) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
					[Constants.KEY_DATA]:
				...
				nextProps[Constants.KEY_TYPE] === types.API_PAID_RESTRO_PAYMENT
			) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
				};
	...
	getDriverList = () => {
		...
		this.props.reqNeedToPayForAllRestro(data, this);
	};
...
RestroPaymentHistory.propTypes = {
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(RestroPaymentHistory));
