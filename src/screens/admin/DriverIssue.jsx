...
import { reqAddDriver, reqOfficeList, reqRegionList, reqCountryList, reqStateList, reqCityList } from '../../actions';
...
class DriverIssue extends BaseComponent {
    constructor(props) {
        super(props)
        this.state = {
    ...
    handleChangeOffice = (selectedOption, isDeleted) => {
        console.log("ghhaghxdxvghasvxdhv", selectedOption);
        if (isDeleted.action == 'clear') {
            this.setState({
                selectedOffice: '',
                [Constants.KEY_OFFICE_ID]: undefined
    ...
    render() {
        ...
        const { date, image, name, address, lat, lng, state, city, zip, phone, email, country,
            errorText, iserror,
        } = this.state;
    ...
    addDriverReq = (data) => {
        ...
        if (this.state[Constants.ACCOUNT_HOLDER_NAME] !== null && this.state[Constants.ACCOUNT_HOLDER_NAME] !== undefined) {
            data[Constants.ACCOUNT_HOLDER_NAME] = this.state[Constants.ACCOUNT_HOLDER_NAME];
        }
        if (this.state[Constants.BANK_NAME] !== null && this.state[Constants.BANK_NAME] !== undefined) {
        ...
        this.props.reqAddDriver(data, this)
    ...
    handleResponse = (nextProps) => {
        ...
        if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
            ...
            if (nextProps[Constants.KEY_TYPE] === types.API_ADD_DRIVER) {
            ...
            else if (nextProps[Constants.KEY_TYPE] === types.API_OFFICE_LIST) {
                let data = nextProps.response.data.officeList;
                console.log('office list data  ::: ', data);
                // let abc = data.map((item, index) => {

                //     data[index].label = item.phone
                // })
                // console.log("new office list,", abc);


                respObj = {
                    [Constants.KEY_SHOW_PROGRESS]: false,
                    [Constants.KEY_OFFICE_LIST]: nextProps[Constants.KEY_RESPONSE][Constants.KEY_DATA][Constants.KEY_OFFICE_LIST]
                };
                this.setState(respObj);
...
export default connect(mapStateToProps, { reqAddDriver, reqOfficeList, reqRegionList, reqCountryList, reqStateList, reqCityList })(withStyles(styles)(DriverIssue));
