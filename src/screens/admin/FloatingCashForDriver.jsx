...
import {
	...
	reqChangeStatusDriver,
	reqFloatingCashForAllDriver,
	reqReceiveFloatingCash,
	reqRegionList,
	reqCountryList,
	reqStateList,
	reqCityList,
...
class FloatingCashForDriver extends BaseComponent {
	constructor(props) {
		super(props);
	...
	firstBtnClick = () => {
		...
		this.props.reqReceiveFloatingCash(
			{
				[Constants.KEY_PAYMENT_RECEIVER_ID]: userData[Constants.KEY_USERID],
				[Constants.KEY_DELIVERY_MAN_ID]:
					selctedFloatingCashItem[Constants.KEY_UNDERSCORE_ID],
				[Constants.KEY_PAYMENT_RECEIVER_USER_TYPE]:
			...
			return (
				<Fragment>
					...
					<div>
						<div class="row">
							<div class="col-sm-8">
								<CommonGridTextField
									...
									style={{ marginTop: 0 }}
									id="restaurantName"
									label={"Search By Driver Name / Mobile Number"}
									fullWidth
						...
						{/* <Grid item xs={12} container direction={'row'} justify={'space-between'} alignItems={'center'} alignContent={'center'}
                            style={{ marginTop: 20 }}
                        >


                            <Grid xs={3} style={{ marginTop: 0, paddingRight: 5, paddingLeft: 0 }}>
                                <CommoanAutocomplete
                            ...
                            <Grid xs={3} style={{ marginTop: 0, paddingRight: 5, }}>
                                <CommoanAutocomplete
					...
					{selctedFloatingCashItem !== undefined && (
						<YesNoDialog
							...
							content={` Receive Amount for Driver ${
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
			if (
				nextProps[Constants.KEY_TYPE] === types.API_FLOATING_CASH_FOR_ALL_DRIVER
			) {
				respObj = {
				...
				nextProps[Constants.KEY_TYPE] === types.API_CHANGE_STATUS_DRIVER
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				this.getDriverList();
			} else if (
				nextProps[Constants.KEY_TYPE] === types.API_DRIVER_DOC_STATUS_CHANGE
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				...
				nextProps[Constants.KEY_TYPE] === types.API_RECEIVE_FLOATING_CASH
			) {
				toast("Success");
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
	...
	getDriverList = () => {
		...
		this.props.reqFloatingCashForAllDriver(data, this);
	};
...
FloatingCashForDriver.propTypes = {
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(FloatingCashForDriver));
