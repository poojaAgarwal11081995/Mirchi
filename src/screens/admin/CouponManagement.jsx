...
let TEXTFIELD_XS = 12;
class CouponManagement extends BaseComponent {
	constructor(props) {
		super(props);
	...
	navigateAddPage() {
		this.props.history.push({
			pathname: Utility.isAdmin()
				? Constants.SCREEN_ADD_COUPON_ADMIN
				: Constants.SCREEN_ADD_COUPON_RESTAURANT,
		});
	}
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_LISTCOUPONCODE) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
					[Constants.KEY_DATA]:
				...
				this.setState(respObj);
			} else if (nextProps[Constants.KEY_TYPE] === types.API_DELETECOUPONCODE) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				toast("coupon code is deleted");
				this.setState(respObj);
				this.getCouponList();
			} else if (
				nextProps[Constants.KEY_TYPE] === types.API_CHANGECOUPONCODESTATUS
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
	...
	getCouponList = () => {
		var data = {
			[Constants.KEY_USERID]:
				userDetails != null ? userDetails[Constants.KEY_USERID] : 0,
			[Constants.KEY_ROLE]:
				userDetails != null ? userDetails[Constants.KEY_ROLE] : "",
		};
		this.props.reqCouponList(data, this);
	};
	...
	deleteCoupon = (couponItem) => {
		...
		this.props.reqDeleteCoupon(data, this);
	};
	updateStatus = (_id, status) => {
		...
		this.props.reqChangeStatusCoupon(data, this);
	};
}
...
export default connect(mapStateToProps, {
	...
	reqCouponList,
	reqDeleteCoupon,
})(withStyles(styles)(CouponManagement));
