...
import {
	...
	reqStateList,
	reqCityList,
	reqChangeUserStatusGrocery,
	reqDeleteUserGrocery,
} from "../../actions";
...
let TEXTFIELD_XS = 12;
class UserListGrocery extends BaseComponent {
	constructor(props) {
		super(props);
		this.state = {
	...
	navigateAddPage() {
		this.props.history.push({
			pathname: Constants.SCREEN_ADD_USER_GROCERY,
		});
	}
	...
	okayBtnClick = () => {
		if (
			...
			this.props.reqDeleteUserGrocery(data, this);
	...
	handleResponse = (nextProps) => {
		...
		if (nextProps.hasOwnProperty(Constants.KEY_RESPONSE)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_USER_LIST_GROCERY) {
				respObj = {
					[Constants.KEY_SHOW_PROGRESS]: false,
					[Constants.KEY_DATA]:
				...
				nextProps[Constants.KEY_TYPE] === types.API_DELETE_USER_GROCERY
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				this.getUserList();
			} else if (
				nextProps[Constants.KEY_TYPE] === types.API_CHANGE_USER_STATUS_GROCERY
			) {
				respObj = { [Constants.KEY_SHOW_PROGRESS]: false };
				this.setState(respObj);
				this.getUserList();
	...
	getUserList = () => {
		...
		this.props.reqUserListGrocery(data, this);
	};
	...
	updateStatus = (_id, status) => {
		...
		this.props.reqChangeUserStatusGrocery(data, this);
	};
...
export default connect(mapStateToProps, {
	...
	reqChangeUserStatusGrocery,
	reqDeleteUserGrocery,
})(withStyles(styles)(UserListGrocery));
