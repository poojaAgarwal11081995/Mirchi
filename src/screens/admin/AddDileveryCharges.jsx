...
import {
	reqChildDeleteDriverDistance,
	reqAddRegion,
	reqCountryList,
	reqStateList,
	reqCityList,
	reqAddDileveryCharge,
} from "../../actions";
...
import DileveryChargeLoadMoreTable from "../../tables/DileveryChargeLoadMoreTable";
...
class AddDileveryCharges extends BaseComponent {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			selectedState: "",
			selectedCity: "",
			[Constants.KEY_DILEVERY_CHARGE_DETAILS]: this.props.location[
				Constants.KEY_DILEVERY_CHARGE_DETAILS
			],
			[Constants.KEY_FARE_LOAD_MORE_DATA]: this.getFareModel(
				this.props.location[Constants.KEY_DILEVERY_CHARGE_DETAILS],
			),
			[Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE]: [],
		};
	...
	getFareModel(data) {
		...
		if (details != undefined) {
			fareOptions = details[Constants.KEY_ORDER_DILEVERY_CHARGES];
		} else {
			fareOptions = [
				{
	...
	deleteDriverFareItem = (_id) => {
		this.setState({
			[Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE]: [
				...this.state[Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE],
				_id,
			],
		});
	};
		...
		if (this.state[Constants.KEY_SHOW_PROGRESS] == true) {
			...
			return (
				<Fragment>
					...
					<Grid container>
						{this.state[Constants.KEY_DILEVERY_CHARGE_DETAILS] == undefined && (
							<Fragment>
								<Grid
									item
									xs={TEXTFIELD_XS}
	...
	checkVaidation = (data) => {
		...
		if (this.state[Constants.KEY_DILEVERY_CHARGE_DETAILS] == undefined) {
			if (Utility.checkDileveryChargeValidation(this.state, this)) {
	...
	fareCheckWithInLoop(data) {
		for (const key in data) {
			if (data.hasOwnProperty(key)) {
				...
				if (Utility.checkDileveryChargeLoadMore(element, this, key, data)) {
					if (key == data.length - 1) {
						this.addDileveryCharge(data);
					}
				} else {
					break;
				}
	...
	addDileveryCharge = (array) => {
		let data = {};
		data[Constants.KEY_ORDER_DILEVERY_CHARGES] = array;

		if (
			this.state[Constants.KEY_DILEVERY_CHARGE_DETAILS] != undefined &&
			[Constants.KEY_DILEVERY_CHARGE_DETAILS] != null &&
			[Constants.KEY_DILEVERY_CHARGE_DETAILS] != ""
		) {
			...
			if (
				this.state[Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE].length > 0
			) {
				data[Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE] = this.state[
					Constants.KEY_ORDER_DILEVERY_CHARGE_IDS_DELETE
				];
			}
		} else {
		...
		this.props.reqAddDileveryCharge(data, this);
	};
	...
	handleResponse = (nextProps) => {
		console.log("iugigiughjughjghjghjghjghjghjghjghjghjghjgh", nextProps);
		...
		if (nextProps.hasOwnProperty(Constants.KEY_SHOW_PROGRESS)) {
			...
			if (nextProps[Constants.KEY_TYPE] === types.API_ADD_LOCALITY) {
				...
				nextProps[Constants.KEY_TYPE] === types.API_ADD_ORDER_DILEVERY_CHARGE
			) {
				...
				nextProps[Constants.KEY_TYPE] === types.API_EDIT_ORDER_DILEVERY_CHARGE
			) {
				toast(nextProps[Constants.KEY_RESPONSE][Constants.KEY_MESSAGE]);
...
export default connect(mapStateToProps, {
	...
	reqStateList,
	reqCityList,
})(withStyles(styles)(AddDileveryCharges));
